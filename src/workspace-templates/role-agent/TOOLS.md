...
| 文件 | 作用 |
|------|------|
| **`manifest.json`** | 能力开关（`version: 2`）：`tools.docs`、`tools.git`、`tools.shell`、`tools.web_search`、`tools.web_scrape`、`tools.scheduling`、`tools.skills`、`tools.knowledge_base`、`tools.feishu` 等。引擎按此过滤模型工具；创建/设置工作区时的勾选会写回此文件。 |
...
| **`scheduling.md`** | **周期调度**（与 `tools.scheduling` 对应）：无人格、结构化定时触发。 |
...
当你调用任何“会改变真实世界状态”的工具（例如写文件/补丁、Git 写操作、重置/删除目录、创建周期调度、创建/修改技能等），都必须遵守：
...
| **`.agent/.clawflow/`** | 主会话、周期调度、爬取与 Hermes DB 等 |
