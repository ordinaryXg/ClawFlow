...
import { StoredConversation } from '../session/session-store';
import {
  resolveWorkspaceRootForWebContents,
  workspaceRootOrUndefined,
} from '../../main/electron-workspace-context';
import { getGlobalClawFlowEngine, type ClawFlowEngineConfig } from '../core/clawflow-engine';
...
import { resolveWebSearchConfig, sanitizeWebSearchForPublic } from '../search/web-search';
...
export function registerClawFlowIPC(config?: ClawFlowEngineConfig): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const { upsertAuthProfile } = await import('../session/auth-store');
  ...
  ipcMain.handle('engineAuth:removeProfile', async (_e, params: { provider: string; profileId: string }) => {
    const { removeAuthProfile } = await import('../session/auth-store');
  ...
  ipcMain.handle(
    ...
    async (_e, params: { provider: string; profileId: string; label?: string; environment?: 'personal' | 'work' | 'custom' }) => {
      const { updateAuthProfileMeta } = await import('../session/auth-store');
