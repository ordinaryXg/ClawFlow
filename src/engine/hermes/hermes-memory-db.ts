...
export async function searchHermesMemory(
  ...
  | { ok: true; hits: HermesMemorySearchHit[]; hybridUsed: boolean }
  | { ok: false; error: string }
> {
  const fts = searchHermesMemoryFts(workspaceRoot, params);
  if (!fts.ok) return fts;
  const db = getOrOpenHermesMemoryDb(workspaceRoot);
  if (!db) return { ok: true, hits: fts.hits, hybridUsed: false };
  ...
  try {
    ...
    const merged = await mergeHybridHermesHits(
      db as unknown as import('./hermes-memory-embeddings').HermesVecDb,
      fts.hits,
      String(params.query ?? ''),
      limit
    );
    return { ok: true, hits: merged.hits, hybridUsed: merged.hybridUsed };
  } catch {
    return { ok: true, hits: fts.hits, hybridUsed: false };
  }
}
...
export async function rebuildHermesSkillFtsIndex(
  workspaceRoot: string
): Promise<HermesMemorySyncResult & { embedded?: number }> {
  invalidateHermesMemoryDbCache(workspaceRoot);
  ...
  if (db) {
    try {
      ...
      if (vec.ok) embedded = vec.embedded;
      else console.warn('[hermes] embedding rebuild failed:', vec.error);
    } catch (e) {
...
export function getHermesMemoryVectorIndexStatus(workspaceRoot: string) {
  const db = getOrOpenHermesMemoryDb(workspaceRoot);
  const { getHermesVectorIndexStatus } = require('./hermes-memory-embeddings') as typeof import('./hermes-memory-embeddings');
  return getHermesVectorIndexStatus(db as import('./hermes-memory-embeddings').HermesVecDb | null);
}
