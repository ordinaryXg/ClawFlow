...
export type HermesVectorIndexStatus = {
  enabled: boolean;
  sqliteVec: boolean;
  docCount: number;
  vectorCount: number;
...
function loadEmbeddingPrefsModule() {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  return require('../../main/prefs/hermes-embedding-prefs') as typeof import('../../main/prefs/hermes-embedding-prefs');
}
...
export function isHermesVectorSearchAvailable(db: HermesVecDb | null): boolean {
  if (!db) return false;
  const { resolveHermesEmbeddingPrefs } = loadEmbeddingPrefsModule();
  if (!resolveHermesEmbeddingPrefs().enabled) return false;
  return tryLoadSqliteVec(db);
}

function vecTableExists(db: HermesVecDb): boolean {
  try {
    db.prepare(`SELECT doc_id FROM memory_vec LIMIT 1`).get();
    return true;
  } catch {
    return false;
  }
...
function recreateVecTable(db: HermesVecDb, dimensions: number): void {
...
function ensureVecTableExists(db: HermesVecDb, dimensions: number): void {
  if (vecTableExists(db)) return;
  recreateVecTable(db, dimensions);
}
...
export function getHermesVectorIndexStatus(db: HermesVecDb | null): HermesVectorIndexStatus {
  const { resolveHermesEmbeddingPrefs } = loadEmbeddingPrefsModule();
  const enabled = resolveHermesEmbeddingPrefs().enabled;
  const sqliteVec = Boolean(db && tryLoadSqliteVec(db));
  let docCount = 0;
  let vectorCount = 0;
  if (db) {
    try {
      docCount = Number((db.prepare(`SELECT COUNT(*) AS c FROM memory_docs`).get() as { c?: number })?.c ?? 0);
    } catch {
      docCount = 0;
    }
    if (vecTableExists(db)) {
      try {
        vectorCount = Number(
          (db.prepare(`SELECT COUNT(*) AS c FROM memory_vec`).get() as { c?: number })?.c ?? 0
        );
      } catch {
        vectorCount = 0;
      }
  ...
  return {
    ...
    docCount,
    vectorCount,
    hybridReady: enabled && sqliteVec && vectorCount > 0,
  };
}
...
/** 增量：清理孤儿向量并为缺失 embedding 的文档补写向量表。 */
export async function syncHermesMemoryEmbeddingsIncremental(
  db: HermesVecDb,
  opts?: { fullRebuild?: boolean }
): Promise<{ ok: true; embedded: number; removed: number } | { ok: false; error: string }> {
  const { resolveHermesEmbeddingPrefs } = await import('../../main/prefs/hermes-embedding-prefs');
  const prefs = resolveHermesEmbeddingPrefs();
  if (!prefs.enabled) return { ok: true, embedded: 0, removed: 0 };
  if (!tryLoadSqliteVec(db)) {
    return { ok: false, error: 'sqlite-vec extension unavailable' };
  }

  if (opts?.fullRebuild) {
    const full = await rebuildHermesMemoryEmbeddings(db);
    return full.ok ? { ok: true, embedded: full.embedded, removed: 0 } : full;
  }

  ensureVecTableExists(db, prefs.dimensions);

  let removed = 0;
  try {
    const run = db.prepare(`DELETE FROM memory_vec WHERE doc_id NOT IN (SELECT id FROM memory_docs)`).run();
    removed = Number(run.changes ?? 0);
  } catch {
    /* ignore */
  }
  ...
  const missing = db
    .prepare(
      `SELECT id, body FROM memory_docs
       WHERE id NOT IN (SELECT doc_id FROM memory_vec)
       ORDER BY id`
    )
    .all() as Array<{ id: number; body: string }>;

  if (!missing.length) return { ok: true, embedded: 0, removed };
  ...
  return { ok: true, embedded, removed };
}
...
export async function searchHermesMemoryVectorAsync(
  ...
  if (!isHermesVectorSearchAvailable(db) || !vecTableExists(db)) return [];
...
export async function mergeHybridHermesHits(
  ...
  query: string,
  limit: number
): Promise<{ hits: HermesMemorySearchHit[]; hybridUsed: boolean }> {
  const { resolveHermesEmbeddingPrefs } = await import('../../main/prefs/hermes-embedding-prefs');
  const prefs = resolveHermesEmbeddingPrefs();
  if (!prefs.enabled || !isHermesVectorSearchAvailable(db) || !vecTableExists(db)) {
    return { hits: ftsHits.slice(0, limit), hybridUsed: false };
  }

  const vectorRows = await searchHermesMemoryVectorAsync(db, query, Math.min(limit * 3, 50));
  if (!vectorRows.length) return { hits: ftsHits.slice(0, limit), hybridUsed: false };
  ...
  const hits = out.length ? out : ftsHits.slice(0, limit);
  return { hits, hybridUsed: true };
}
