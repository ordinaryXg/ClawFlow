...
- 数据落在 **`.clawflow-data/todo-triggers.v1.json`**（工作区根；与 `.agent/` 分离）。
- 主进程调度；创建/改删后 `workspace_todo_list` 可核验。
...
**勿**：把模糊大任务塞进 `actionText`（到点原样注入，不会澄清）；用待办替代即时执行；未 list 回执就声称已创建/已删。
...
| 工具 | 要点 |
|------|------|
| `workspace_todo_list` | 无参；返回 id、title、enabled、status、nextFireAt、repeat、submitToModel |
| `workspace_todo_create` | 必填 `title`、`actionText`、`submitToModel`、`repeat`（`once` \| `interval` \| `cron`）；`interval` 需 `intervalMinutes`；`cron` 需 `cron`，可选 `cronTz`（如 `Asia/Shanghai`） |
| `workspace_todo_update` | 必填 `id`；其余字段同 create，按需传 |
| `workspace_todo_remove` | 必填 `id` |
...
2. **创建后**：`workspace_todo_list` 核对 id、`nextFireAt`。
