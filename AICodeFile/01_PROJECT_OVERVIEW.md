...
- **核心依赖**：本机 OpenClaw CLI（`openclaw ...`），通过主进程执行并以 IPC 暴露给渲染进程
...
- **环境依赖**：运行机必须能找到 `openclaw`（PATH 或未来可配置路径）
- **跨平台**：文档与脚本避免硬编码盘符路径；文档统一用 `${REPO_ROOT}`

## 术语表（Glossary）

- **OpenClaw CLI**：命令行工具 `openclaw`，负责 gateway、技能、连接器、对话等能力的底层实现
- **Gateway**：OpenClaw 的本地服务/进程（项目中提供启停与状态查询）
