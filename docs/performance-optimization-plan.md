...
**问题分析**:
```typescript
// 当前实现 (src/engine/provider-router.ts)
...
async function registerNonCriticalIPC() {
  ...
  const { registerGatewayIPC } = await import('./engine/gateway-daemon');
...
**当前问题**:
```typescript
// src/engine/hermes-memory-db.ts
...
**优化方案**:
```typescript
// src/engine/hermes-memory-db.ts (优化后)
