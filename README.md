...
- **`.agent/`** 位于**工作区根目录**；**`.clawflow-launcher-stash/`** 在应用缓存 `workspaces/<sha256>/`（见 `workspace-blob-store.ts`）。应用启动时**不**自动创建或绑定工作区；用户从侧栏添加文件夹后才会 `setActive`。打开工作区时会同步 stash 布局、清理遗留 **`.subagent/`**（工作区委派已移除；系统子 Agent 在应用缓存）。
